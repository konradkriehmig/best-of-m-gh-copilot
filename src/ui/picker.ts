...
async function confirm(plan: RunPlan): Promise<boolean> {
  ...
  const choice = await vscode.window.showWarningMessage(
    `Run ${total} parallel agent sessions?`,
    {
      ...
      detail:
        ...
        `session in AI credits. Each runs with --allow-all-tools in its own worktree. ` +
        `Worktrees share your filesystem and credentials and are not a security boundary.`,
    },
    'Run',
  );
  return choice === 'Run';
...
export async function buildRunPlan(repoRoot: string): Promise<RunPlan | undefined> {
  ...
  return (await confirm(plan)) ? plan : undefined;
}
